import { Scan, realScans } from "./store";
...
export type ItemStat = {
  ...
  pctChange: number;
  dollarChange: number;
  appearances: number; // total times seen (incl. baseline)
...
export function freqPurchasesPerYear(f: string | null) {
  if (!f) return 26;
  return (FREQ_PER_MONTH[f] ?? 4) * 12;
}

export function aggregateItems(scans: Scan[]): ItemStat[] {
  const byKey = new Map<string, { entries: { date: string; price: number; fromBaseline: boolean; name: string }[] }>();
...
export function inflationScore(stats: ItemStat[]) {
...
export function painIndex(stats: ItemStat[], totalSpendDelta: number) {
  if (!stats.length) return 0;
...
export function painLabel(score: number) {
...
export type Confidence = { level: "low" | "medium" | "high"; label: string };
...
export function confidence(scans: Scan[], stats: ItemStat[]): Confidence {
  ...
  if (real >= 3 && tracked >= 10) return { level: "medium", label: "Building confidence — keep scanning" };
...
export function totalSpendBaselineVsCurrent(stats: ItemStat[]) {
  // Sum of (current - baseline) across items
